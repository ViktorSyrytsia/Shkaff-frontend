...
const CheckoutForm = () => {
  ...
  const handleOnSubmit = () => {
    ...
    const isValidateDelivery =
      deliveryMethod === 2 || deliveryMethod === 4 || deliveryMethod === 5
        ? Object.values(delivery).every(val => val.isValid)
        : true;
    const isValidateAddress = deliveryMethod === 3 ? Object.values(address).every(val => val.isValid) : true;

    if (!isValidateCustomer || !isValidateDelivery || !isValidateAddress || !connectionMethod || !deliveryMethod) {
      setError(true);
      return;
    }
    ...
    switch (id) {
      case 'customer': {
        setCustomer({
          ...
          [name]: {
            value: value,
            isValid: checkoutFieldValidate(name, value.trim())
          }
        });
        return;
      }
    ...
    <Form>
      {CUSTOMER_INPUTS_DATA.map((data, i) => (
        <Form.Input
          key={i}
          error={
            error && !customer[data.name].isValid
              ? {
                  content: data.error,
                  pointing: 'below'
                }
      ...
      <Form.Field required className={'checkbox'}>
        ...
        {error && !connectionMethod && <span className={'checkout-error'}>Виберіть спосіб зв‘язку нижче</span>}
      </Form.Field>
      ...
      />
      {error && !deliveryMethod && <div className={'checkout-error'}>Виберіть спосіб доставки</div>}
        ...
        <>
          {POST_DELIVERY_INPUTS_DATA.map((data, i) => (
            <Form.Input
              key={i}
              error={
                error && !delivery[data.name].isValid
                  ? {
                      content: data.error,
                      pointing: 'below'
                    }
        ...
        <>
          ...
          {CURRIER_DELIVERY_INPUTS_DATA.map((data, i) => (
            <Form.Input
              key={i}
              error={
                error && !address[data.name].isValid
                  ? {
                      content: data.error,
                      pointing: 'below'
                    }
